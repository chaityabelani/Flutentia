      ...
      <div className="relative z-10 max-w-[1200px] mx-auto px-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-12 items-start reveal">
          ...
          <div className="relative bg-gradient-card border border-accent-primary/20 rounded-xl3 p-6 sm:p-12 flex flex-col justify-between min-h-[300px]">
            ...
            <blockquote className="text-[1.1rem] text-slate-100 leading-[1.8] italic mt-8 mb-9 flex-1">
              {t.quote}
            </blockquote>
            ...
            <div className="flex items-center gap-3.5">
              ...
              <div>
                <div className="font-bold text-[0.95rem] text-slate-100">{t.name}</div>
                <div className="text-[0.8rem] text-slate-600">{t.role}</div>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-2">
            {testimonials.map((item, i) => (
              ...
              >
                <div className="w-10 h-10 rounded-full flex items-center justify-center text-[0.8rem] font-bold text-white flex-shrink-0"
                     style={{ background: `linear-gradient(135deg,${item.color},${item.color}88)` }}>
                  {item.avatar}
                </div>
                <div>
                  <div className="text-[0.875rem] font-semibold mb-0.5">{item.name}</div>
                  <div className="text-[0.75rem] text-slate-600">{item.role}</div>
                </div>
              </button>
            ))}
          </div>
